...
The current research.md system prompt is a single sentence. It must be expanded — especially the Constraints block — to satisfy the PRD NFR ("common home-cooking techniques") and prevent the model generating recipes that require specialist equipment or unrealistic time.
