...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "both visibility and planning — tracking expiry is the trigger, recipe generation is the real payoff"
    - topic: "key insight"
      decision: "AI recipe generation closes the last mile: trackers stop at 'warning', this goes to 'here's what to cook'"
    - topic: "primary persona scope"
      decision: "single household — one person managing their own fridge"
    - topic: "auth model"
      decision: "email + password; no social auth; no magic link"
    - topic: "role model"
      decision: "flat — every logged-in user has the same access to their own data; no roles"
    - topic: "mvp timeline"
      decision: "3 weeks after-hours; achievable; hard deadline 2026-06-07"
    - topic: "guardrails"
      decision: "data isolation per account; inventory consistency (approval screen = exact delete set)"
    - topic: "domain rule"
      decision: "urgency-driven prioritization using a 3-day at-risk window; recipe must use at least one at-risk product"
    - topic: "at-risk window"
      decision: "3 days or fewer — expiring today, tomorrow, or the day after"
    - topic: "NFRs"
      decision: "AI output practically usable; browser support (2 latest major versions); user data isolated at data layer"
    - topic: "product type"
      decision: "web-app"
    - topic: "target scale"
      decision: "small — just me or a handful"
    - topic: "non-goals"
      decision: "no product editing; no quantity tracking; no notifications; no barcode/receipt scanning; no search/filter; no multi-user sharing; no offline support; no separate recipe detail page"
...
### Recipe Generation
- FR-007: A user can request an AI-generated recipe that prioritizes their soonest-expiring products. Priority: must-have
...
- A user's product and recipe data must not be accessible to any other user, including through direct URL construction or session manipulation — isolation is enforced at the data layer, not only in the UI.

## Business Logic

When a user requests a recipe, the system identifies products expiring within 3 days from their inventory and instructs an AI to generate a recipe that uses at least one of those at-risk products as a required ingredient.

The inputs are: the user's current inventory (product names + expiry dates) and today's date. The system computes which products fall within the 3-day at-risk window and passes them — alongside the full inventory for context — to the AI. The AI's output is a recipe where at least one at-risk product appears as a required ingredient. The user sees the recipe and the specific products it would consume. If they approve, those products are removed from inventory and the recipe is saved.

The rule is urgency-driven prioritization: the application decides what to cook around based on what's about to go bad, not based on the user's preferences or arbitrary recipe browsing. An at-risk product expiring today holds priority over a safe product expiring in two weeks, regardless of what the user "feels like" cooking.
...
1. **What happens when no at-risk products exist?** The user's inventory may have no products expiring within 3 days. Does the AI generate a recipe from the full inventory anyway, or does the UI block recipe generation? — Owner: user. Block: yes (affects FR-007 and business logic).
2. **How many products does the AI receive?** Is the full inventory passed to the AI, or only the at-risk subset? Passing the full inventory gives more recipe variety; passing only at-risk products enforces the waste-reduction intent more strictly. — Owner: user. Block: no (implementation detail, but shapes the prompt design).
